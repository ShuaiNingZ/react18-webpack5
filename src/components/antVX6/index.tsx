...
const FlowNode = ({node}) => {
  return (
      <Dropdown
          ...
          trigger={['contextMenu']}
      >
        <div className={style["custom-react-node"]}>
          {node.data.name}
        </div>
      </Dropdown>
  )
  ...
  componentDidMount() {
    ...
    data.edges.forEach(item => {
      if (!map.has(item.fromNode)) {
        ...
        map.set(item.fromNode, [...map.get(item.fromNode), item.toNode])
      }
    })
    ...
    const fromJSON = {
      nodes: data.nodes.reduce((prev, cur, i) => {
        ...
        const curFromKeyVal = [...map.entries()].find(([key, value]) => {
          return value.includes(cur.nodeId)
        });
        const curFrom = prev.find(item => item.id === curFromKeyVal[0]);
